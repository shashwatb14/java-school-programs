    ...
    */
    ...
    public static String[] calculate(int[][] scores) {    // calculate the average grade for each student, then
        ...
        for (int i = 0, n = scores.length; i < n; i++) {
            double sum = 0;
            int l = scores[i].length;
            for (int j = 0; j < l; j++) {
                sum += scores[i][j];
            }
            averages[i] = scoreToGrade((sum / l));
        }
        return averages;
    }
    ...
    public static double classAverage(int[][] scores) {    // calculate the class average based on the 2D array with students scores
        ...
        for (int[] score : scores) {
            for (int j = 0; j < score.length; j++, c++) {
                sum += score[j];
            }
        }
        average = sum / c;
    ...
    public static void main(String args[]) {
        String[] students = {"Able", "Adam", "Alan", "Geoff", "John", "Karl",
                "Keane", "Louie", "Michael", "Selina"};
        // Jon -> John (3 letter = buggy output)
